...
class CubeEnv:
    def __init__(self, name, state_type, initial_state, is_goal_pred,
                 ...
                 render_func, render_action_func, to_action_func,
                 encoded_shape, encode_func):
        self.name = name
        ...
        self._render_action_func = render_action_func
        self._to_action_func = to_action_func
    ...
    def render_action(self, action):
        assert isinstance(action, self.action_enum)
        return self._render_action_func(action)
        
    def to_action(self, form):
        return self._to_action_func(form)

    def render_action_list(self, lis):
        return ",".join(map(lambda x: self.render_action(self.action_enum(x)), lis))
